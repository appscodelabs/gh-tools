...
libbuild.REPO_ROOT = expandvars('$GOPATH') + '/src/github.com/appscodelabs/gh-tools'
...
def update_registry():
    vf = libbuild.REPO_ROOT + '/dist/gh-tools/versions.json'
    bucket = libbuild.BUCKET_MATRIX.get(libbuild.ENV, libbuild.BUCKET_MATRIX['dev'])
    call('gsutil cp {0}/binaries/gh-tools/versions.json {1}'.format(bucket, vf))
    vj = {}
    ...
    call("gsutil cp {1} {0}/binaries/gh-tools/versions.json".format(bucket, vf))
    call('gsutil acl ch -u AllUsers:R -r {0}/binaries/gh-tools/versions.json'.format(bucket))

    lf = libbuild.REPO_ROOT + '/dist/gh-tools/latest.txt'
    libbuild.write_file(lf, BUILD_METADATA['version'])
    call("gsutil cp {1} {0}/binaries/gh-tools/latest.txt".format(bucket, lf))
    call('gsutil acl ch -u AllUsers:R -r {0}/binaries/gh-tools/latest.txt'.format(bucket))
